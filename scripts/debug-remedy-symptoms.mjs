...
const files = [
  'supabase/seed.sql',
  join(BASE, '008_expanded_remedies.sql'),
  join(BASE, '009_symptom_remedies.sql'),
  join(BASE, '010_body_pain_symptoms.sql'),
  join(BASE, '012_symptom_expansion.sql'),
  join(BASE, '013_database_repair.sql'),
  join(BASE, '014_symptom_remedies_expansion.sql'),
];
const allSQL = files.map(f => readFileSync(f, 'utf-8')).join('\n');
