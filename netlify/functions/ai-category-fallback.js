...
export async function handler(event) {
  ...
  try {
    ...
    if (!rawQuery || !isValidQuery(rawQuery)) return buildResponse(400, { error: 'Valid query is required.' });
    ...
    return buildResponse(200, { category: category || fallbackCategory(query) });
  } catch {
    return buildResponse(500, { error: 'Unable to match symptom.' });
  }
}
